...
mod gpu {
    ...
    #[test]
    fn gpu_render_is_deterministic_and_nonempty() {
        ...
        let a = match render_frame(&comp, FrameIndex(0), backend.as_mut(), &mut assets) {
            Ok(v) => v,
            Err(e) if e.to_string().contains("no gpu adapter available") => return,
            Err(e) => panic!("unexpected gpu render error: {e}"),
        };
        let b = match render_frame(&comp, FrameIndex(0), backend.as_mut(), &mut assets) {
            Ok(v) => v,
            Err(e) if e.to_string().contains("no gpu adapter available") => return,
            Err(e) => panic!("unexpected gpu render error: {e}"),
        };
