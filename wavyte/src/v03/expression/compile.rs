use crate::v03::expression::bind::{BindCtx, bind_expr};
use crate::v03::expression::error::ExprError;
use crate::v03::expression::lower::lower_to_bytecode;
...
use crate::v03::normalize::ir::{ExprSourceIR, NormalizedComposition, ValueTypeIR};
...
pub(crate) fn compile_expr_program(
    ...
    for e in &entries {
        if let PropertyProgram::Expr(bc) = &e.program {
            for op in &bc.ops {
                if let crate::v03::expression::bytecode::Op::LoadProp(pid) = op {
                    required.insert(*pid);
                }
            }
        }
    ...
    match p {
        PropertyProgram::Expr(bc) => bc
            ...
            .filter_map(|op| match op {
                crate::v03::expression::bytecode::Op::LoadProp(pid) => Some(*pid),
                _ => None,
            })
            .collect(),
...
mod tests {
    ...
    fn compile_orders_deps_before_dependents() {
        ...
        let norm = crate::v03::normalize::pass::normalize(&def).unwrap();
